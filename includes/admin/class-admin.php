...
if (!class_exists('WC_NFe_Admin')) {
    ...
    class WC_NFe_Admin
    {
        ...
        public function __construct()
        {
            ...
            add_action('woocommerce_save_product_variation', [$this, 'save_variations_fields'], 10, 2);
            add_action('woocommerce_product_data_panels', [$this, 'product_data_fields']);
            add_action('woocommerce_process_product_meta', [$this, 'product_data_fields_save']);

            add_action('admin_enqueue_scripts', [$this, 'register_enqueue_css']);
            add_action('woocommerce_after_dashboard_status_widget', [$this, 'nfe_status_widget_order_rows']);

            // NFe issue triggers.
            add_action('woocommerce_order_status_pending', [$this, 'issue_trigger']);
            add_action('woocommerce_order_status_on-hold', [$this, 'issue_trigger']);
            add_action('woocommerce_order_status_processing', [$this, 'issue_trigger']);
            add_action('woocommerce_order_status_completed', [$this, 'issue_trigger']);

            // WooCommerce Subscriptions Support.
            if (class_exists('WC_Subscriptions')) {
                add_action('processed_subscription_payments_for_order', [$this, 'issue_trigger']);
                add_action('woocommerce_renewal_order_payment_complete', [$this, 'issue_trigger']);
            }
        }
			...
			<div class="nfe-details">
				<p>
					...
					<?php if (!empty($nfe['amountNet'])) { ?>
						<?php echo wc_price($nfe['amountNet'], ['currency' => $order->get_currency()]); // WPCS: XSS ok.?>
					<?php } ?>
					<br />
